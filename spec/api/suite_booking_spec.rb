...
describe("Test Case Booking") do
  ...
  context ("PATCH") do
    let(:post_token) { auth_requests.create_token(obj_token) }
    let(:obj_patch_book) { attributes_for(:update_booking) }
    let(:post_booking) { booking_requests.post_booking(obj_booking) }
    let(:booking_id) { post_booking["bookingid"] }

    it "update firstname and lastname" do
      token = post_token["token"]
      obj_booking[:firstname] = obj_patch_book[:firstname]
      obj_booking[:lastname] = obj_patch_book[:lastname]
      patch_booking_response = booking_requests.patch_booking(booking_id, obj_booking, token)
      expect(patch_booking_response.code).to eq(200)
      expect(patch_booking_response["firstname"]).to eq(obj_patch_book[:firstname])
      expect(patch_booking_response["lastname"]).to eq(obj_patch_book[:lastname])
    end
  end
